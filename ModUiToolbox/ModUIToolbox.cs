...
public sealed class MyDuMod : IMod
{
    ...
    private const string McpBridgeEventFileLegacyName = "bridge-events.ndjson";
    private const int McpBridgeEventFileMaxBytes = 512 * 1024;
    ...
    public Task Initialize(IServiceProvider isp)
    {
        ...
        {
            logger.LogWarning(
                "UIToolbox could not resolve server root from AppContext.BaseDirectory ({BaseDir}); falling back to current directory ({CurrentDir})",
        ...
        {
            logger.LogError("UIToolbox payload JS is empty; injection action will fail");
        }
        else
        {
        ...
        {
            logger.LogError("UIToolbox Lua probe JS is empty; probe action will fail");
        }
        else
        {
    ...
    private async Task WatchIdeImportFile()
    {
        ...
        while (true)
        {
            ...
            {
                logger.LogDebug(ex, "UIToolbox IDE import watcher tick failed");
            }
        }
    }
    ...
    private async Task ProcessIdeImportFile(string importPath, Dictionary<string, IdeImportWatchState> watchStates)
    {
        ...
        {
            logger.LogDebug(ex, "UIToolbox failed to read IDE import file {ImportPath}", importPath);
            return;
        }
        ...
        {
            logger.LogDebug(ex, "UIToolbox failed to parse IDE import file {ImportPath}", importPath);
            return;
        }
        ...
        var injectCode =
            ...
            "var probe=window.__UI_TOOLBOX_LUA_PROBE_STATE__;" +
    ...
    private async Task WatchMcpBridgeCommands()
    {
        while (true)
        {
            ...
            {
                logger.LogDebug(ex, "UIToolbox MCP bridge command watcher tick failed");
            }
        }
    }
    ...
    private async Task ProcessMcpBridgeCommandFile(string commandPath)
    {
        ...
        {
            logger.LogWarning(ex, "UIToolbox failed to process MCP bridge command file {CommandPath}", commandPath);
    ...
    private void MoveMcpBridgeCommandToProcessed(string commandPath, string commandId)
    {
        ...
        {
            logger.LogDebug(ex, "UIToolbox failed to move processed MCP bridge command {CommandId}", commandId);
        }
    }
    ...
    private async Task<JObject> BuildServerChatSnapshotPayload(string commandId, ulong playerId, JObject payload)
    {
        ...
        try
        {
            transaction = await sql.Transaction("uitoolbox_server_chat_snapshot");
        ...
        {
            logger.LogWarning(ex, "UIToolbox server_chat snapshot failed for player {PlayerId}", playerId);
...
ORDER BY table_schema, table_name, ordinal_position";
    ...
    public async Task TriggerAction(ulong playerId, ModAction action)
    {
        switch (action.actionId)
        {
            ...
            case ActionIngestPacket:
                ...
                return;
            default:
                logger.LogDebug("UIToolbox ignoring action {ActionId} from player {PlayerId}", action.actionId, playerId);
                return;
        }
    }
    ...
    private async Task InjectStylesheetFromTargetFile(ulong playerId)
    {
        if (!TryReadTargetStylesheetHref(out var href, out var error))
        {
            ...
            logger.LogWarning("UIToolbox target stylesheet URL missing: {Reason}", error);
            return;
        }
    ...
    private async Task InjectLuaProbePayload(ulong playerId, ModAction action)
    {
        ...
        var injectCode = $"window.__UI_TOOLBOX_LUA_PROBE_CONFIG={config.ToString(Newtonsoft.Json.Formatting.None)};\n{luaProbeScript}";
    ...
    private async Task InjectPayload(ulong playerId, JObject config, string notifyMessage, string modeTag)
    {
        ...
        {
            logger.LogWarning(
                "UIToolbox runtime override does not appear to support mode {Mode}; falling back to embedded payload for this injection",
                requestedMode);
    ...
    private async Task<bool> InjectJavaScript(ulong playerId, string injectCode, string modeTag, string? notifyMessage = null, bool notifyPlayer = true)
    {
        try
        {
            ...
            logger.LogInformation(
                "UIToolbox injected payload for player {PlayerId} (mode={Mode}, bytes={Bytes})",
                playerId,
                modeTag,
                injectCode.Length);
        ...
        {
            logger.LogWarning(ex, "UIToolbox injection failed for player {PlayerId}", playerId);
            if (notifyPlayer)
            {
    ...
    private Task AppendMcpBridgeEvent(string sourceKind, string eventType, ulong? playerId, JObject payload, string? boardId = null)
    {
        try
        {
            ...
            var serializedLine = line.ToString(Newtonsoft.Json.Formatting.None) + Environment.NewLine;
            lock (mcpBridgeEventWriteGate)
            {
                var eventPath = ResolveMcpBridgeEventFilePathForAppend(Encoding.UTF8.GetByteCount(serializedLine));
                File.AppendAllText(eventPath, serializedLine, Encoding.UTF8);
            }
        }
        catch (Exception ex)
        {
            logger.LogDebug(ex, "UIToolbox failed to append MCP bridge event {EventType}", eventType);
        }

        return Task.CompletedTask;
    }

    private string ResolveMcpBridgeEventFilePathForAppend(int appendUtf8Bytes)
    {
        Directory.CreateDirectory(mcpBridgeEventsDirectory);

        var dateKey = DateTime.UtcNow.ToString("yyyyMMdd", CultureInfo.InvariantCulture);
        var todaysFiles = Directory
            .GetFiles(mcpBridgeEventsDirectory, "bridge-events-*.ndjson", SearchOption.TopDirectoryOnly)
            .Select(path => new
            {
                Path = path,
            ...
            {
                entry.Path,
                Parsed = TryParseMcpBridgeEventFileName(entry.FileName, out var parsedDateKey, out var parsedSequence)
                    ? new { DateKey = parsedDateKey, Sequence = parsedSequence }
                    : null
            })
            .Where(entry => entry.Parsed is not null && string.Equals(entry.Parsed.DateKey, dateKey, StringComparison.Ordinal))
            .OrderBy(entry => entry.Parsed!.Sequence)
            .ToArray();

        if (todaysFiles.Length == 0)
        {
            return Path.Combine(mcpBridgeEventsDirectory, BuildMcpBridgeEventFileName(dateKey, 0));
        }

        var latest = todaysFiles[^1];
        var latestLength = new FileInfo(latest.Path).Length;
        if (latestLength + appendUtf8Bytes <= McpBridgeEventFileMaxBytes)
        {
            return latest.Path;
        }

        return Path.Combine(mcpBridgeEventsDirectory, BuildMcpBridgeEventFileName(dateKey, latest.Parsed!.Sequence + 1));
    }

    private static string BuildMcpBridgeEventFileName(string dateKey, int sequence)
    {
        if (sequence <= 0)
        {
            return $"bridge-events-{dateKey}.ndjson";
        }

        return $"bridge-events-{dateKey}-{sequence.ToString("D3", CultureInfo.InvariantCulture)}.ndjson";
    }

    private static bool TryParseMcpBridgeEventFileName(string fileName, out string dateKey, out int sequence)
    {
        dateKey = "";
        sequence = 0;
        if (string.Equals(fileName, McpBridgeEventFileLegacyName, StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }
        ...
        if (!stem.StartsWith("bridge-events-", StringComparison.OrdinalIgnoreCase))
        {
            return false;
        }

        var suffix = stem.Substring("bridge-events-".Length);
        if (suffix.Length < 8)
        {
            return false;
        }

        var parsedDateKey = suffix.Substring(0, 8);
        if (!parsedDateKey.All(char.IsDigit))
        {
            return false;
        }
        ...
        if (suffix.Length != 12 || suffix[8] != '-')
        {
            return false;
        }

        var parsedSequence = suffix.Substring(9, 3);
        if (!parsedSequence.All(char.IsDigit))
        {
            return false;
        }

        dateKey = parsedDateKey;
        sequence = int.Parse(parsedSequence, CultureInfo.InvariantCulture);
        return true;
    }
    ...
    private void EnsureTargetStylesheetFile()
    {
        ...
        {
            logger.LogWarning(ex, "UIToolbox failed to create stylesheet target file: {TargetFile}", targetStylesheetFilePath);
        }
    }
    ...
    private bool TryReadTargetStylesheetHref(out string href, out string error)
    {
        ...
        {
            error = ex.Message;
            logger.LogWarning(ex, "UIToolbox failed to read stylesheet target file: {TargetFile}", targetStylesheetFilePath);
            return false;
        }
    }
    ...
    private async Task IngestPacket(ulong playerId, string? payload)
    {
        ...
        {
            logger.LogDebug("UIToolbox received empty packet payload from player {PlayerId}", playerId);
            return;
        }
        ...
        {
            logger.LogInformation(
                "UIToolbox dump complete for player {PlayerId}. dumpId={DumpId}, output={DumpPath}",
                playerId,
                dumpId,
                dumpPath);
        ...
        {
            logger.LogWarning(
                "UIToolbox dump fatal event from player {PlayerId}. dumpId={DumpId}, output={DumpPath}",
                playerId,
                dumpId,
                dumpPath);
    ...
    private JObject? TryAssembleLuaMcpResultChunk(ulong playerId, JObject packetData)
    {
        ...
        {
            logger.LogDebug("UIToolbox ignored lua_mcp_result_chunk without packetId for player {PlayerId}", playerId);
            return null;
        }
        ...
        {
            logger.LogDebug(
                "UIToolbox ignored invalid lua_mcp_result_chunk metadata for player {PlayerId}: packetId={PacketId}, part={Part}, total={Total}",
                playerId,
                packetId,
                part,
                total);
        ...
        lock (state.Gate)
        {
            ...
            catch (Exception ex)
            {
                logger.LogWarning(
                    ex,
                    "UIToolbox failed to parse reassembled lua_mcp_result chunk payload for player {PlayerId}: packetId={PacketId}",
                    playerId,
                    packetId);
                return null;
            }
    ...
    private async Task ProcessThemeCatalogRequest(ulong playerId, JObject packetData)
    {
        ...
        {
            logger.LogDebug(ex, "UIToolbox failed to load theme catalog for player {PlayerId}", playerId);
    ...
    private string LoadEmbeddedScriptSafe(string resourceHint, string scriptLabel)
    {
        try
        {
            ...
            {
                logger.LogError("UIToolbox {ScriptLabel} resource not found: {ResourceHint}", scriptLabel, resourceHint);
                return "";
            }
            ...
            if (stream is null)
            {
                logger.LogError("UIToolbox {ScriptLabel} resource stream is null: {ResourceName}", scriptLabel, resourceName);
                return "";
            }
        ...
        {
            logger.LogError(ex, "UIToolbox failed to load embedded script {ScriptLabel}", scriptLabel);
            return "";
        }
    }
    ...
    private void EnsureRuntimeOverrideFile(string path, string seedScript, string scriptLabel)
    {
        try
        {
            ...
            logger.LogInformation(
                "UIToolbox created runtime override for {ScriptLabel} at {Path}. Edit this file while the server is running; next injection uses the updated script.",
                scriptLabel,
                path);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "UIToolbox failed to create runtime override file for {ScriptLabel}: {Path}", scriptLabel, path);
        }
    }
    ...
    private JObject LoadLuaProbeRuntimeModuleStateContainer()
    {
        ...
        {
            logger.LogWarning(ex, "UIToolbox failed to read runtime lua probe module state from {Path}", runtimeLuaProbeRuntimeModulesStatePath);
            return new JObject
            {
                ["modules"] = new JObject()
            };
    ...
    private void SaveLuaProbeRuntimeModuleStateContainer(JObject payload)
    {
        ...
        {
            logger.LogWarning(ex, "UIToolbox failed to persist runtime lua probe module state to {Path}", runtimeLuaProbeRuntimeModulesStatePath);
        }
    }
    ...
    private async Task ProcessHudEditorIdeExport(ulong playerId, JObject packetData)
    {
        ...
        {
            logger.LogWarning(ex, "UIToolbox failed to write Lua Painter IDE export for player {PlayerId}", playerId);
        ...
        try
        {
            ...
            foreach (var moduleDir in Directory.GetDirectories(runtimeLuaProbeRuntimeModulesDirectoryPath))
            {
                try
                {
                    ...
                    {
                        logger.LogWarning(
                            "UIToolbox ignored runtime lua probe module {ModuleId} because entry file is invalid: {EntryPath}",
                            id,
                            entryPath);
                        continue;
                    }
                ...
                {
                    logger.LogWarning(ex, "UIToolbox failed to load runtime lua probe module from {ModuleDir}", moduleDir);
                }
            }
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "UIToolbox failed to enumerate runtime lua probe modules from {ModuleDir}", runtimeLuaProbeRuntimeModulesDirectoryPath);
        }
    ...
    private string ResolveLuaProbeScript(out bool usingRuntimeOverride, out bool usingRuntimeModules)
    {
        ...
        if (!string.IsNullOrWhiteSpace(moduleScript))
        {
            ...
            logger.LogInformation(
                "UIToolbox using runtime lua probe modules from {ModulesDir} ({ModuleCount} files)",
                runtimeLuaProbeModulesDirectoryPath,
                moduleCount);
            return moduleScript;
        }
    ...
    private string ResolveRuntimeModuleScript(string moduleDirectoryPath, string manifestPath, string scriptLabel, out int moduleCount)
    {
        ...
        try
        {
            ...
            {
                logger.LogWarning(
                    "UIToolbox runtime module manifest for {ScriptLabel} has no module entries: {ManifestPath}",
                    scriptLabel,
                    manifestPath);
                return "";
            }
            ...
            foreach (var entry in moduleEntries)
            {
                ...
                {
                    logger.LogWarning(
                        "UIToolbox runtime module entry escapes module directory for {ScriptLabel}: {Entry}",
                        scriptLabel,
                        entry);
                    return "";
                }
                ...
                {
                    logger.LogWarning(
                        "UIToolbox runtime module file missing for {ScriptLabel}: {ModulePath}",
                        scriptLabel,
                        modulePath);
                    return "";
                }
                ...
                {
                    logger.LogWarning(
                        "UIToolbox runtime module file is empty for {ScriptLabel}: {ModulePath}",
                        scriptLabel,
                        modulePath);
                    return "";
                }
        ...
        catch (Exception ex)
        {
            logger.LogWarning(
                ex,
                "UIToolbox failed to read runtime module manifest for {ScriptLabel} from {ManifestPath}; using single-file fallback",
                scriptLabel,
                manifestPath);
            return "";
        }
    ...
    private string ResolveRuntimeScript(string runtimePath, string embeddedFallbackScript, string scriptLabel, out bool usingRuntimeOverride)
    {
        ...
        try
        {
            if (File.Exists(runtimePath))
            {
                ...
                logger.LogWarning(
                    "UIToolbox runtime override for {ScriptLabel} is empty at {Path}; using embedded fallback",
                    scriptLabel,
                    runtimePath);
            }
        }
        catch (Exception ex)
        {
            logger.LogWarning(
                ex,
                "UIToolbox failed to read runtime override for {ScriptLabel} from {Path}; using embedded fallback",
                scriptLabel,
                runtimePath);
        }
    ...
    private async Task Notify(ulong playerId, string message)
    {
        ...
        {
            logger.LogDebug(ex, "UIToolbox notification failed for player {PlayerId}", playerId);
        }
    }
}
