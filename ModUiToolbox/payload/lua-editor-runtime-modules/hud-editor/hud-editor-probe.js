...
(function hudEditorCore() {
  ...
  function getRuntimeCtx() {
    return WIN.__HUD_EDITOR_RUNTIME_CTX__ || null;
  }

  function getPersistentValue(key, fallbackValue) {
    var runtimeCtx = getRuntimeCtx();
    if (!runtimeCtx || typeof runtimeCtx.getState !== "function") {
      return typeof fallbackValue === "undefined" ? null : cloneJsonValue(fallbackValue, fallbackValue);
    }
    return runtimeCtx.getState(key, fallbackValue);
  }

  function setPersistentValue(key, value) {
    var runtimeCtx = getRuntimeCtx();
    if (!runtimeCtx || typeof runtimeCtx.setState !== "function") {
      return false;
    }
      ...
      el("div", { id: "properties-panel", className: "docked-panel" }, [
        ...
        el("div", { className: "panel-content" }, [
          ...
          el("div", { className: "panel-tab-page active", dataset: { panelTabPage: "shape" } }, [
            ...
            el("div", { className: "prop-row", dataset: { propRow: "strokeWidth" } }, [
              el("label", { textContent: "Stroke Width" }),
            ...
            el("div", { className: "panel-tab-page", dataset: { panelTabPage: "text" } }, [
              ...
              el("label", { textContent: "Text Color" }),
              el("button", {
                type: "button",
                className: "prop-color-btn",
                title: "Text color",
              ...
              el("div", { className: "prop-inline-field", dataset: { propRow: "textAlign" } }, [
                el("label", { textContent: "Horizontal" }),
...
(function hudEditorShapesPanel() {
  ...
  var el = APP.el;
  var qs = APP.qs;
  var getPersistentValue = APP.getPersistentValue;
  var setPersistentValue = APP.setPersistentValue;
  ...
  function normalizeStoredPanelPos(value) {
    if (!value || typeof value !== "object") return null;
    var left = null;
    var top = null;
    if (typeof value.leftPx !== "undefined") {
      left = toFiniteNumber(value.leftPx, null);
    } else if (typeof value.left !== "undefined" && value.left !== "") {
      left = toFiniteNumber(value.left, null);
    }
    if (typeof value.topPx !== "undefined") {
      top = toFiniteNumber(value.topPx, null);
    } else if (typeof value.top !== "undefined" && value.top !== "") {
      top = toFiniteNumber(value.top, null);
    }
    if (left === null && top === null) return null;
    return { leftPx: left, topPx: top };
  }

  function persistPanelPos(pos) {
    if (!pos || typeof pos !== "object") return;
    if (typeof setPersistentValue === "function") {
      try { setPersistentValue(POS_KEY, pos); } catch (e) { /* ignore */ }
    }
    try {
  ...
  function readStoredPanelPos() {
    ...
    if (typeof getPersistentValue === "function") {
      try {
        pos = normalizeStoredPanelPos(getPersistentValue(POS_KEY, null));
      } catch (e) { /* ignore */ }
      if (pos) return pos;
    }
    try {
      ...
      pos = normalizeStoredPanelPos(JSON.parse(saved));
      if (pos && typeof setPersistentValue === "function") {
        try { setPersistentValue(POS_KEY, pos); } catch (e) { /* ignore */ }
      }
      return pos;
    } catch (e) { /* ignore */ }
  ...
  function savePanelPos(panel) {
    persistPanelPos({
      leftPx: toFiniteNumber(panel && panel.style ? panel.style.left : null, null),
      topPx: toFiniteNumber(panel && panel.style ? panel.style.top : null, null),
    });
  }

  function restorePanelPos(panel) {
    var storedPos = readStoredPanelPos();
    var left = null;
    var top = 72;
    if (storedPos) {
      if (storedPos.leftPx !== null) left = storedPos.leftPx;
      if (storedPos.topPx !== null) top = storedPos.topPx;
    }

    var editorScreen = panel.closest('[data-screen="editor"]');
    var editorWidth = editorScreen ? editorScreen.clientWidth : 0;
    var editorHeight = editorScreen ? editorScreen.clientHeight : 0;

    // Default: right side of editor when layout metrics are ready.
    if (left === null && editorWidth > 0) {
      left = editorWidth - (panel.offsetWidth || 240) - 12;
    }
    if (left === null) left = 12;

    if (editorWidth > 0) {
      left = Math.max(0, Math.min(left, editorWidth - (panel.offsetWidth || 240)));
    }
    if (editorHeight > 0) {
      top = Math.max(0, Math.min(top, editorHeight - (panel.offsetHeight || 80)));
    }
...
(function hudEditorPropertiesPanel() {
  ...
  function getCollapsedPanelHeight(panel) {
    ...
    if (!headerHeight && typeof header.getBoundingClientRect === "function") {
      headerHeight = Math.ceil(header.getBoundingClientRect().height || 0);
    }
    return headerHeight > 0 ? (headerHeight + 2) + "px" : "";
  }

  function rememberExpandedPanelHeight(panel) {
    if (!panel || panel.classList.contains("collapsed")) return;
    var expandedHeight = panel.style && panel.style.height ? String(panel.style.height).trim() : "";
    if (!expandedHeight && typeof panel.getBoundingClientRect === "function") {
      var rect = panel.getBoundingClientRect();
      if (rect && rect.height > 0) {
        expandedHeight = Math.ceil(rect.height) + "px";
      }
    }
    if (expandedHeight) {
      panel.dataset.expandedHeight = expandedHeight;
    }
  }

  function restoreExpandedPanelHeight(panel) {
    if (!panel) return;
    var expandedHeight = panel.dataset ? panel.dataset.expandedHeight : "";
    panel.style.height = expandedHeight || "";
    panel.style.overflowY = "auto";
  }

  function applyCollapsedState(panel, collapsed, options) {
    if (!panel) return;
    var opts = options || {};
    if (collapsed && opts.captureExpanded !== false) {
      rememberExpandedPanelHeight(panel);
    }
    ...
    if (btn) btn.textContent = collapsed ? "\u25B8" : "\u25BE";
    if (collapsed) {
      var collapsedHeight = getCollapsedPanelHeight(panel);
      if (collapsedHeight) {
        panel.style.height = collapsedHeight;
      }
      panel.style.overflowY = "hidden";
      return;
    }
    restoreExpandedPanelHeight(panel);
  }
  ...
  function restoreCollapse() {
    ...
    applyCollapsedState(panel, collapsed, { captureExpanded: collapsed });
  }
  ...
  function restorePanelPos(panel) {
    ...
    var editorHeight = editorScreen ? editorScreen.clientHeight : 0;
    if (editorWidth > 0) {
      left = Math.max(0, Math.min(left, editorWidth - (panel.offsetWidth || 340)));
    }
    if (editorHeight > 0) {
      top = Math.max(0, Math.min(top, editorHeight - (panel.offsetHeight || 80)));
    }
  ...
  function onPanelActionClick(e) {
    var button = e.target.closest("#properties-panel [data-action]");
    if (!button) return;
    ...
    if (action === "delete-element" && APP.state.selectedElementId) {
      APP.emit("delete-element", APP.state.selectedElementId);
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  function attachPanelListeners() {
    ...
    panel.removeEventListener("click", onPanelActionClick);
    ...
    panel.addEventListener("click", onPanelActionClick);
...
(function hudEditorDialogs() {
  ...
  function buildDialogs() {
    ...
    var renameD = el("div", {
      ...
      el("div", { className: "dialog" }, [
        el("div", { className: "dialog-header" }, [
          el("h3", { textContent: "Rename Layout" }),
          el("button", {
            className: "dialog-close",
            dataset: { action: "close-rename" },
            textContent: "\u00D7",
          }),
        ]),
        el("div", { className: "dialog-content" }, [
          el("label", { textContent: "Layout Name" }),
          el("input", {
            type: "text",
            id: "rename-name",
            className: "saveas-input",
          ...
          el("p", { className: "save-note", textContent: "Updates the selected saved layout name" }),
        ]),
        el("div", { className: "dialog-footer" }, [
          el("button", {
            className: "btn secondary",
  ...
  function onDialogClick(e) {
    ...
    if (action === "close-load") {
      ...
      var renameDialog = qs("#dialog-rename");
      var renameInput = qs("#rename-name");
      var renameName = renameInput ? renameInput.value.trim() : "";
      var scriptId = renameDialog && renameDialog.dataset ? String(renameDialog.dataset.scriptId || "") : "";
      if (scriptId && renameName) {
        APP.emit("rename-layout-confirm", {
          id: scriptId,
          name: renameName
        });
        hideDialog();
  ...
  APP.on("rename-dialog-open", function (payload) {
    var scriptId = payload && payload.id ? String(payload.id) : "";
    if (!scriptId) return;
    showDialog("rename");
    var dialog = qs("#dialog-rename");
    var input = qs("#rename-name");
    if (dialog && dialog.dataset) {
      dialog.dataset.scriptId = scriptId;
    }
    if (input) {
      input.value = payload && payload.name ? String(payload.name) : "";
