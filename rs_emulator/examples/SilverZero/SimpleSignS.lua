...
-- Intent-faithful port of the HTML SVG source
...
local masterScale = layout.scale
local masterW = resolutionX
local masterH = resolutionY

-- Background fill with circuit-color-A
...
for _, pathData in ipairs(circuitBPaths) do
    SZ.drawPath(masterLayer, layout, pathData, theme.circuitB, 2.0)
end
...
for _, pathData in ipairs(circuitCPaths) do
    SZ.drawPath(masterLayer, layout, pathData, theme.circuitC, 1.8)
end
...
for _, dot in ipairs(dots) do
    SZ.drawDot(masterLayer, layout, dot.x, dot.y, dot.r, theme.circuitC)
end
...
-- Outer board outline path (in primary color)
...
local scaledBoardLayout = {
    screenW = resolutionX,
    screenH = resolutionY,
    sourceW = 231,
    sourceH = 156,
    scale = boardScale * 0.1,
    x = boardX,
    y = boardY,
}

SZ.drawPath(boardLayer, scaledBoardLayout, boardOutlinePath, theme.primary, 3.0)

-- Inner tick marks in highlight color
...
for _, pathData in ipairs(highlightPaths) do
    SZ.drawPath(boardLayer, scaledBoardLayout, pathData, theme.highlight, 2.0)
end
...
local logoLayout = {
    ...
    x = logoX,
    y = logoY - logoSize * 0.5,
}

local logoPaths = {
    { d = "m16295 8596.6-174.4 100.8-0.9 0.67-462.4-266.92-462.6-267.53v-207.78h0.2l959.1 553.89 140.6 81.01 5.7 3.39-5.3 2.47", fill = theme.primary },
    { d = "m16300 11435-146 84.1-479.1 276.5-480 277.4h-0.2v-207.8l462.6-267.5 462.4-266.9 0.9 0.6 174.4 100.8 5.3 2.5-0.3 0.3", fill = theme.primary },
    { d = "m14838 11767-1-0.6-1460.5-843.2v-1687.8l1461.2-843.53 0.3-0.31 0.2 0.31h0.4l1461.1 843.53v1687.8l-1461.1 843.5h-0.4l-0.2 0.3m0.1-57.5 1411.8-815.1v-1630.2l-1411.8-815.08-1411.8 815.08v1630.2l1411.8 815.1", fill = theme.primary },
    { d = "m16300 12640-0.8 674.4-220.2 127.3-6.2 1.5v-1608.6l6.2 3.6 220.2 127.2 1.2 0.8-0.4 673.8", fill = theme.primary },
    { d = "m15244 12924 1.1-0.7 527.3 304.4 527.7 305.1v237h-0.4l-1093.7-631.8-166.9-96.2 6-2.8 198.9-115", fill = theme.highlight },
    { d = "m15772 9616-527.3 304.42-1.1-0.77-198.9-114.95-6-2.83 104.7-60.63 960.6-554.52 122.6-70.88 73.1-41.95v236.97l-527.7 305.14", fill = theme.highlight },
    { d = "m16295 10363-169.8-98.1-0.6-519.94-0.2-519.45 0.8-0.6 169.8-98.08 4.8-2.71v1240.1l-4.8-1.2", fill = theme.highlight },
    { d = "m15804 11308-495.6-286.4v-572.3l495.6-286.4 496 286.4v572.3l-496 286.4m0-61.2 442.9-255.8v-511l-442.9-255.8-442.6 255.7v511.1l442.6 255.8", fill = theme.primary },
    { d = "m15981 9856.9-795.1-458.77-794.9 458.77v918.12l794.9 458.8 795.1-458.8zm-794.5 1744.5-0.6 0.3-1113.4-642.9v-1285.7l1113.4-642.87 0.6 0.33 1113.1 642.54v1285.7l-1113.1 642.6", fill = theme.highlight },
}

for _, pathInfo in ipairs(logoPaths) do
    SZ.drawPath(logoLayer, logoLayout, pathInfo.d, pathInfo.fill, 2.0)
end
...
local textY = resolutionY * 0.14 + (resolutionY * 0.52) * 0.5
...
addText(textLayer, textFont, "SilverZero's Lab", textX, textY)
