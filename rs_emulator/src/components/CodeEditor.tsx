...
const RS_COMPLETIONS: { label: string; kind: string; insertText: string; detail: string }[] = [
  ...
  { label: "loadFont", kind: "Function", insertText: "loadFont(\"${1:RobotoMono}\", ${2:16})", detail: "Load a preset RenderScript font" },
  ...
  { label: "getTime", kind: "Function", insertText: "local t = getTime()", detail: "Get elapsed runtime in seconds" },
  { label: "getResolution", kind: "Function", insertText: "local w, h = getResolution()", detail: "Get current screen resolution in pixels" },
  { label: "getAvailableFontCount", kind: "Function", insertText: "local count = getAvailableFontCount()", detail: "Get number of preset fonts" },
  { label: "getAvailableFontName", kind: "Function", insertText: "local fontName = getAvailableFontName(${1:1})", detail: "Get preset font name by 1-based index" },
  { label: "getRenderCost", kind: "Function", insertText: "local cost = getRenderCost()", detail: "Get current render cost for this frame" },
  { label: "getRenderCostMax", kind: "Function", insertText: "local maxCost = getRenderCostMax()", detail: "Get render cost budget limit" },
  ...
  { label: "requestAnimationFrame", kind: "Function", insertText: "requestAnimationFrame(${1:1})", detail: "Schedule another run in N display frames" },
...
const RS_SNIPPETS = [
  ...
  },
  {
    ...
    insertText: [
      ...
      "local t = getTime()",
      "local r = math.min(w / 4, h / 4)",
      "local cx = w / 2 + r * math.cos(t)",
      "local cy = h / 2 + r * math.sin(t)",
      "",
      "setNextFillColor(layer, 0.3, 0.7, 1, 1)",
      "addCircle(layer, cx, cy, 16)",
      "",
      "requestAnimationFrame(1)",
    ].join("\n"),
    detail: "Animated circle from the in-game help pattern",
  },
  {
    label: "rs-render-cost-overlay",
    insertText: [
      "local w, h = getResolution()",
      "local layer = createLayer()",
      ...
      "addText(layer, font, text, 16, h - 8)",
    ].join("\n"),
    detail: "Small render-cost profiler overlay",
  },
];
