...
ScreenLayoutEditor.INPUT_KIND = "slei"
ScreenLayoutEditor.OUTPUT_CHUNK_KIND = "oc"
ScreenLayoutEditor.OUTPUT_CHUNK_ACK_KIND = "oca"
ScreenLayoutEditor.OUTPUT_CHUNK_SIZE = 880
...
function ScreenLayoutEditor.serializeOutputEnvelope(document, serializedDocument, hash)
    ...
    return string.format(
        '{"k":%s,"r":%d,"g":%s,"d":%s,"sx":%s,"sy":%s,"si":%s}',
...
function ScreenLayoutEditor.commitDocument(state)
    ...
    dp(string.format("commit r=%d b=%d", state.document.revision or 0, #(state.lastOutputEnvelope or "")))
...
local function readJsonInitialDocument(inputText, screenWidth, screenHeight)
    local jsonModule = nil
    if type(json) == "table" and type(json.decode) == "function" then
        jsonModule = json
    elseif type(require) == "function" then
        local requireOk, requiredModule = pcall(require, "dkjson")
        if requireOk and type(requiredModule) == "table" and type(requiredModule.decode) == "function" then
            jsonModule = requiredModule
        end
    end

    if type(jsonModule) ~= "table" or type(jsonModule.decode) ~= "function" then
        dp("ip no_json")
        return nil
    end

    local decodeOk, decoded = pcall(jsonModule.decode, inputText)
    if not decodeOk or type(decoded) ~= "table" then
        dp("ip bad_json")
        return nil
    end

    local inputKind = decoded.kind or decoded.k
    if inputKind ~= ScreenLayoutEditor.INPUT_KIND then
        dp("ip kind")
        return nil
    end

    local inputScreenWidth = math.max(1, math.floor(numberOrNil(decoded.screenWidth or decoded.sx) or screenWidth or 1920))
    local inputScreenHeight = math.max(1, math.floor(numberOrNil(decoded.screenHeight or decoded.sy) or screenHeight or 1080))
    local documentValue = decoded.document or decoded.d
    local patchValue = decoded.minimal or decoded.m
    local initialDocument = nil

    if type(documentValue) == "string" and documentValue ~= "" then
        initialDocument = ScreenLayoutEditor.deserializeDocument(documentValue, inputScreenWidth, inputScreenHeight)
    elseif type(patchValue) == "string" and patchValue ~= "" then
        initialDocument = ScreenLayoutEditor.deserializeLayoutPatch(patchValue, inputScreenWidth, inputScreenHeight)
    elseif type(documentValue) == "table" then
        initialDocument = ScreenLayoutEditor.normalizeDocument(documentValue, inputScreenWidth, inputScreenHeight)
    end
    ...
    initialDocument.revision = math.max(0, math.floor(numberOrNil(decoded.revision or decoded.r) or initialDocument.revision or 0))
    local selectedId = decoded.selectedId or decoded.si
    if type(selectedId) == "string" and selectedId ~= "" then
        initialDocument.selectedId = selectedId
    end
    return {
        document = initialDocument,
        token = table.concat({
            "j",
            tostring(initialDocument.revision or 0),
            tostring(initialDocument.selectedId or ""),
            tostring(initialDocument.screenWidth or inputScreenWidth),
            tostring(initialDocument.screenHeight or inputScreenHeight)
        }, ":")
    }
end

local function readChunkedInitialDocument(inputText, screenWidth, screenHeight)
    local token, index36, count36, revision36, sx36, sy36, selectedId, chunkText =
        inputText:match("^c|([^|]*)|([^|]*)|([^|]*)|([^|]*)|([^|]*)|([^|]*)|([^|]*)|(.*)$")
    if not token then
        return nil
    end
    ...
    local revision = tonumber(revision36, 36) or 0
    local inputScreenWidth = tonumber(sx36, 36) or screenWidth or 1920
    local inputScreenHeight = tonumber(sy36, 36) or screenHeight or 1080
    if not chunkIndex or not chunkCount or chunkIndex < 1 or chunkCount < 1 or chunkIndex > chunkCount then
        dp("ip bad_chunk")
        return nil
    end

    local startupState = type(SCREEN_LAYOUT_EDITOR_INPUT_STATE) == "table" and SCREEN_LAYOUT_EDITOR_INPUT_STATE or {}
    SCREEN_LAYOUT_EDITOR_INPUT_STATE = startupState
    if startupState.token ~= token or startupState.count ~= chunkCount then
        startupState.token = token
        startupState.count = chunkCount
        startupState.parts = {}
        startupState.document = nil
        startupState.revision = revision
        startupState.screenWidth = inputScreenWidth
        startupState.screenHeight = inputScreenHeight
        startupState.selectedId = selectedId
    end
    startupState.parts[chunkIndex] = chunkText

    local ack = string.format("a|%s|%s|%s", token, encodeBase36(chunkIndex), encodeBase36(chunkCount))
    for index = 1, chunkCount do
        if type(startupState.parts[index]) ~= "string" then
            return {
                pending = true,
                token = token,
                ack = ack
    ...
    if type(startupState.document) ~= "table" then
        local documentText = table.concat(startupState.parts, "")
        local document, documentError = ScreenLayoutEditor.deserializeDocument(
            documentText,
            inputScreenWidth,
            inputScreenHeight
        )
        if type(document) ~= "table" then
            dp("ip no_doc:" .. tostring(documentError))
            return {
                pending = true,
                token = token,
                ack = ack
        ...
        document.revision = math.max(0, math.floor(revision or document.revision or 0))
        if type(selectedId) == "string" and selectedId ~= "" then
            document.selectedId = selectedId
        end
        startupState.document = document
    end

    return {
        document = startupState.document,
        token = token,
        ack = ack
    }
end

local function readOutputTransferAck(inputText)
    if type(inputText) ~= "string" or inputText == "" then
        return nil
    end
    local token, indexText, countText =
        inputText:match("^" .. ScreenLayoutEditor.OUTPUT_CHUNK_ACK_KIND .. "|([^|]*)|([^|]*)|([^|]*)$")
    if not token then
        return nil
    end
    local chunkIndex = tonumber(indexText)
...
local function getInitialDocumentSeed(inputText, screenWidth, screenHeight)
    ...
    local chunkedResult = readChunkedInitialDocument(inputText, screenWidth, screenHeight)
    if chunkedResult then
        return chunkedResult
    end

    return readJsonInitialDocument(inputText, screenWidth, screenHeight)
end

local function buildOutputTransfer(text, revision)
    local envelopeText = tostring(text or "")
    if envelopeText == "" then
        return nil
    end
    local chunkSize = math.max(256, math.floor(numberOrNil(ScreenLayoutEditor.OUTPUT_CHUNK_SIZE) or 880))
    local chunks = {}
    local startIndex = 1
    while startIndex <= #envelopeText do
        chunks[#chunks + 1] = envelopeText:sub(startIndex, startIndex + chunkSize - 1)
    ...
    return {
        kind = ScreenLayoutEditor.OUTPUT_CHUNK_KIND,
        token = table.concat({
            tostring(math.max(0, math.floor(numberOrNil(revision) or 0))),
            tostring(ScreenLayoutEditor.hashText(envelopeText)),
            tostring(#envelopeText)
        }, "."),
        revision = math.max(0, math.floor(numberOrNil(revision) or 0)),
        chunks = chunks,
        count = #chunks,
        nextIndex = 1,
        text = envelopeText
...
local function buildOutputChunk(transfer)
    if type(transfer) ~= "table" then
        return ""
    end
    local chunkIndex = math.max(1, math.floor(numberOrNil(transfer.nextIndex) or 1))
    local chunkText = type(transfer.chunks) == "table" and tostring(transfer.chunks[chunkIndex] or "") or ""
    if chunkText == "" then
        return ""
    end
    return table.concat({
        ScreenLayoutEditor.OUTPUT_CHUNK_KIND,
        "|",
        tostring(transfer.token or ""),
        "|",
        tostring(chunkIndex),
        "|",
        tostring(math.max(1, math.floor(numberOrNil(transfer.count) or 1))),
        "|",
        chunkText
    })
end
...
local function getState()
    ...
    local outputAck = readOutputTransferAck(inputText)
    if type(SCREEN_LAYOUT_EDITOR_STATE) ~= "table" or not SCREEN_LAYOUT_EDITOR_STATE.initialized then
        local state = ScreenLayoutEditor.createState(
            screenWidth,
            screenHeight,
            startup and startup.document or nil
        )
        state.initialized = true
        state.startupToken = startup and startup.document and startup.token or nil
        state.startupPending = startup and startup.pending == true or false
        state.startupAck = startup and startup.ack or ""
        state.outputAck = outputAck
    ...
    state.startupAck = startup and startup.ack or ""
    state.outputAck = outputAck
    if startup and type(startup.document) == "table" and state.startupToken ~= startup.token then
        state.document = ScreenLayoutEditor.normalizeDocument(startup.document, screenWidth, screenHeight)
        state.documentDirty = false
        state.lastOutputEnvelope = ""
        state.outputTransfer = nil
        state.operation = nil
        state.startupToken = startup.token
    end
    state.startupPending = startup and startup.pending == true and state.startupToken ~= startup.token or false
...
local function runRenderScript()
    ...
    if type(state.startupAck) == "string" and state.startupAck ~= "" then
        local ackOk, ackErr = pcall(setOutput, state.startupAck)
        if not ackOk then
            dp("ack " .. tostring(ackErr))
        end
    ...
    if not state.startupPending then
    ...
    if state.startupPending then
        requestAnimationFrame(1)
        return
    end

    local outputTransfer = state.outputTransfer
    local outputAck = state.outputAck
    if type(outputTransfer) == "table" and type(outputAck) == "table" and outputAck.token == tostring(outputTransfer.token or "") then
        if outputAck.chunkIndex >= math.max(1, math.floor(numberOrNil(outputTransfer.nextIndex) or 1)) then
            outputTransfer.nextIndex = outputAck.chunkIndex + 1
        end
        if outputAck.chunkIndex >= math.max(1, math.floor(numberOrNil(outputTransfer.count) or 1)) then
            dp(string.format("oc ok c=%d", outputAck.chunkCount or 0))
            state.outputTransfer = nil
            state.outputDeliveredToken = outputTransfer.token
            state.lastOutputEnvelope = ""
        end
    end
    ...
    if envelope ~= "" then
        local maxChunkSize = math.max(256, math.floor(numberOrNil(ScreenLayoutEditor.OUTPUT_CHUNK_SIZE) or 880))
        if #envelope > maxChunkSize then
            local transfer = state.outputTransfer
            if type(transfer) ~= "table" or transfer.text ~= envelope then
                transfer = buildOutputTransfer(envelope, state.document and state.document.revision or 0)
                state.outputTransfer = transfer
                if type(transfer) == "table" then
                    dp(string.format("oc tx c=%d b=%d", transfer.count or 0, #envelope))
                end
            end
            if type(transfer) == "table" and math.max(1, math.floor(numberOrNil(transfer.nextIndex) or 1)) <= math.max(1, math.floor(numberOrNil(transfer.count) or 1)) then
                local chunkText = buildOutputChunk(transfer)
                if chunkText ~= "" then
                    local ok, err = pcall(setOutput, chunkText)
                    if ok then
                        state.os = "oc"
                    else
                        state.os = oe(err)
    ...
    if state.operation or cursorPressed or cursorDown or cursorReleased or state.outputTransfer then
        nextFrameDelay = 1
    end
