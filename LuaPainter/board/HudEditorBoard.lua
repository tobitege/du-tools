...
local function appendScreenPainterCommandSpec(commands, methodName, commandCode)
    if type(commandCode) ~= "string" or commandCode == "" then
        return
    end
    commands[#commands + 1] = {
        method = tostring(methodName or "bx"),
        code = commandCode
    }
end

local function appendScreenElementPainterCommands(commands, element)
    if type(element) ~= "table" or element.visible == false then
        return
    end
    local elementType = normalizeScreenElementType(element.type)
    if elementType == "text" then
        appendScreenPainterCommandSpec(commands, "tx", serializeScreenTextCommand(element))
        return
    end
    if elementType == "bezierArc" then
        appendScreenPainterCommandSpec(commands, "bz", serializeScreenBezierCommand(element))
        appendScreenPainterCommandSpec(commands, "tx", serializeScreenTextCommand(element))
        return
    end
    if elementType == "image" then
        appendScreenPainterCommandSpec(commands, "ig", serializeScreenImageCommand(element))
        appendScreenPainterCommandSpec(commands, "tx", serializeScreenTextCommand(element))
        return
    end
    if elementType == "line" then
        appendScreenPainterCommandSpec(commands, "ln", serializeScreenLineCommand(element))
        appendScreenPainterCommandSpec(commands, "tx", serializeScreenTextCommand(element))
        return
    end
    if elementType == "boxRounded" then
        appendScreenPainterCommandSpec(commands, "br", serializeScreenShapeCommand(element, elementType))
    elseif elementType == "circle" then
        appendScreenPainterCommandSpec(commands, "ci", serializeScreenShapeCommand(element, elementType))
    elseif elementType == "triangle" then
        appendScreenPainterCommandSpec(commands, "tr", serializeScreenShapeCommand(element, elementType))
    elseif elementType == "quad" then
        appendScreenPainterCommandSpec(commands, "qd", serializeScreenShapeCommand(element, elementType))
    else
        appendScreenPainterCommandSpec(commands, "bx", serializeScreenShapeCommand(element, elementType))
    end
    appendScreenPainterCommandSpec(commands, "tx", serializeScreenTextCommand(element))
end

local function collectScreenPainterCommands(document)
    ...
    for index = 1, #elements do
        appendScreenElementPainterCommands(commands, elements[index])
    end
    return commands
end
...
function HudEditorBoard.buildRenderScript(document)
    local doc = type(document) == "table" and document or state.document
    local commands = collectScreenPainterCommands(doc)
    if type(doc) ~= "table" or not commands then
    ...
    local lines = {
        ...
        "    c=c or d",
        "    setNextFillColor(l,N(c[1],d[1]),N(c[2],d[2]),N(c[3],d[3]),N(c[4],d[4]))",
        "end",
        "local function SC(l,c,d)",
        "    c=c or d",
        "    setNextStrokeColor(l,N(c[1],d[1]),N(c[2],d[2]),N(c[3],d[3]),N(c[4],d[4]))",
        "end",
        "local function X(ctx,v)",
        "    return N(v,0)*ctx.sx",
        "end",
        "local function Y(ctx,v)",
        "    return N(v,0)*ctx.sy",
        "end",
        "local function S(ctx,v)",
        "    return N(v,0)*ctx.sc",
        "end",
        "function P.bg(r,g,b)",
        ...
        "    local dw=math.max(1,N(w,rx))",
        "    local dh=math.max(1,N(h,ry))",
        "    local sx=rx/dw",
        "    local sy=ry/dh",
        "    return {sx=sx,sy=sy,sc=math.min(sx,sy)}",
        "end",
        "function P.st(ctx,l,c)",
        "    local r=N(c.rot,0)",
        "    if r~=0 then",
        "        setNextRotation(l,r)",
        "    end",
        ...
        "    local blur=sh and N(sh.b,0) or 0",
        "    if sca and blur>0 then",
        "        setNextShadow(l,math.max(0,S(ctx,blur)),N(sca[1],0),N(sca[2],0),N(sca[3],0),N(sca[4],0))",
        "    end",
        "end",
        "function P.bx(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    FC(l,c.f,{0.2,0.2,0.2,1})",
        "    SC(l,c.s,{1,1,1,1})",
        "    setNextStrokeWidth(l,math.max(0,S(ctx,c.sw)))",
        "    addBox(l,X(ctx,c.x),Y(ctx,c.y),X(ctx,c.w),Y(ctx,c.h))",
        "end",
        "function P.br(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    FC(l,c.f,{0.2,0.2,0.2,1})",
        "    SC(l,c.s,{1,1,1,1})",
        "    setNextStrokeWidth(l,math.max(0,S(ctx,c.sw)))",
        "    addBoxRounded(l,X(ctx,c.x),Y(ctx,c.y),X(ctx,c.w),Y(ctx,c.h),math.max(0,S(ctx,c.r)))",
        "end",
        "function P.ci(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    FC(l,c.f,{0.2,0.2,0.2,1})",
        "    SC(l,c.s,{1,1,1,1})",
        "    setNextStrokeWidth(l,math.max(0,S(ctx,c.sw)))",
        ...
        "    local w=X(ctx,c.w)",
        "    local h=Y(ctx,c.h)",
        "    local qi=N(c.qi,0.125)",
        "    addQuad(l,x,y,x+w*(1-qi),y+h*qi,x+w,y+h,x+w*qi,y+h*(1-qi))",
        "end",
        "function P.bz(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    SC(l,c.s,{1,1,1,1})",
        "    setNextStrokeWidth(l,math.max(1,S(ctx,c.sw or 2)))",
        "    local x=X(ctx,c.x)",
        "    local y=Y(ctx,c.y)",
        "    local w=X(ctx,c.w)",
        "    local h=Y(ctx,c.h)",
        ...
        "end",
        "function P.ln(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    SC(l,c.s,{1,1,1,1})",
        "    setNextStrokeWidth(l,math.max(1,S(ctx,c.sw or 2)))",
        "    local x=X(ctx,c.x)",
        "    local y=Y(ctx,c.y)",
        "    addLine(l,x,y,x+X(ctx,c.w),y+Y(ctx,c.h))",
        "end",
        "function P.ig(ctx,l,c)",
        "    P.st(ctx,l,c)",
        "    if c.f then",
        "        FC(l,c.f,{0.2,0.2,0.2,1})",
        "    end",
        "    local image=GI(c.src)",
        "    if not image then",
        "        return",
        "    end",
        "    addImage(l,image,X(ctx,c.x),Y(ctx,c.y),X(ctx,c.w),Y(ctx,c.h))",
        "end",
        "function P.tx(ctx,l,c)",
        "    local lines=c.l",
        ...
        "    local y=Y(ctx,c.y)+Y(ctx,c.h)*0.5-(blockHeight-s)*0.5",
        "    if va==\"top\" then",
        "        y=Y(ctx,c.y)+12*ctx.sc",
        "    elseif va==\"bottom\" then",
        "        y=Y(ctx,c.y)+Y(ctx,c.h)-12*ctx.sc-(blockHeight-s)",
        "    end",
        ...
        "end",
        "P.bg(0,0,0)",
        "local ctx=P.ctx(" .. luaNumber(doc.screenWidth, 1920) .. "," .. luaNumber(doc.screenHeight, 1080) .. ")",
        "local layer=P.ly()"
    }
    for index = 1, #commands do
        local command = commands[index]
        append(lines, "")
        append(lines, "P." .. tostring(command.method) .. "(ctx, layer, " .. tostring(command.code) .. ")")
    end
    append(lines, "")
