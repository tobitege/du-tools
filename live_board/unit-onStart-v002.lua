...
SLE_LOG_LEVEL = "info" --export ScreenLayoutEditor chat level: debug, info, error
SLE_BUILD = "0329a" --export Short live build marker for SLE diagnostics
...
local function BuildScreenLayoutEditorRenderSource()
    ...
    source = cutBetween(
        ...
        "function ScreenLayoutEditor.deserializeLayoutPatch("
    )
    source = cutBetween(
        source,
        "function ScreenLayoutEditor.parseOutputEnvelope(",
        "function ScreenLayoutEditor.findElement("
    )
    return source
end

function BuildEditableRenderScript(initialDocumentState)
    local parts = {}
    ...
    if type(initialDocumentState) == "table" then
        local patchText = tostring(initialDocumentState.patch or "")
        if patchText ~= "" then
            add("SLE_IP=")
            add(q(patchText))
            add("\n")
            add("SLE_IR=")
            add(tostring(math.max(0, math.floor(n(initialDocumentState.revision, 0)))))
            add("\n")
            add("SLE_IS=")
            add(q(initialDocumentState.selectedId or ""))
            add("\n")
            add("SLE_IW=")
            add(tostring(math.max(1, math.floor(n(initialDocumentState.screenWidth, 1920)))))
            add("\n")
            add("SLE_IH=")
            add(tostring(math.max(1, math.floor(n(initialDocumentState.screenHeight, 1080)))))
            add("\n")
        end
    end
    add(BuildScreenLayoutEditorRenderSource())
...
function ScreenLayoutEditor.commitDocument(state, elementId)
    ...
    state.lastOutputEnvelope = ScreenLayoutEditor.serializeTransportDelta(
        state.document,
        elementId,
        state.metrics and state.metrics.screenWidth or 1920,
        state.metrics and state.metrics.screenHeight or 1080
    )
...
local function runRenderScript()
    ...
    if not SCREEN_LAYOUT_EDITOR_BOOT_SENT then
        local bootTarget = ScreenLayoutEditor.findElement(state.document, state.document.selectedId or "main_canvas")
        local bootCode = "boot"
        if type(bootTarget) == "table" then
            bootCode = string.format(
                "boot:%s.%s.%s.%s",
                encodeBase36(bootTarget.x),
                encodeBase36(bootTarget.y),
                encodeBase36(bootTarget.w),
                encodeBase36(bootTarget.h)
            )
        end
        local bootOk, bootErr = pcall(
            setOutput,
            string.format(
                "p|%s|%s|0",
...
if useFormEditorSlice then
    ...
    if screenLayoutEditor then
        ...
        local renderScript = BuildEditableRenderScript(initialDocumentState)
